...
Sistema que recebe solicitações de atendimento, distribui automaticamente para o time e atendente corretos, e expõe métricas para gestores acompanharem a operação com atualização em tempo real via SSE.
...
- Backend expõe um snapshot REST para gestores acompanharem:
...
- Stream SSE envia eventos de criação, atribuição e finalização em tempo real.
